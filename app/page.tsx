...
import { marqueeRows, siteConfig } from "@/lib/site";
...
export default function HomePage() {
  return (
    <PageShell variant="home" pageLabel={siteConfig.navLabel}>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-6 pb-24 pt-8 md:px-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:gap-14 lg:pt-16">
          <div className="space-y-8">
            <div className="flex flex-wrap gap-2">
              {siteConfig.home.pills.map((pill) => (
                <Pill key={pill}>{pill}</Pill>
              ))}
            </div>
            <div className="space-y-6">
              <h1 className="max-w-5xl text-5xl font-semibold leading-[0.94] tracking-[-0.06em] text-white md:text-7xl lg:text-[6.4rem]">
                {siteConfig.home.headline}
                <span className="block text-white">{siteConfig.home.headlineEmphasis}</span>
              </h1>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {siteConfig.home.stats.map(([label, value]) => (
          ...
          <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.05] shadow-2xl shadow-black/35 backdrop-blur-md">
            <div className="relative aspect-[4/5] overflow-hidden bg-[linear-gradient(135deg,rgba(255,255,255,0.18),rgba(255,255,255,0.04)_42%,transparent_65%)]">
              ...
              <div className="absolute inset-x-5 bottom-5 space-y-4 rounded-[1.4rem] border border-white/10 bg-black/48 p-5 backdrop-blur-md">
                <div className="flex items-center gap-2 text-sm text-white/70"><Play className="h-4 w-4" /> {siteConfig.home.reelTitle}</div>
                <div className="text-2xl font-medium tracking-tight text-white">{siteConfig.home.reelPlaceholder}</div>
                <div className="flex flex-wrap gap-2 text-[11px] uppercase tracking-[0.24em] text-white/56">
                  {siteConfig.home.reelTags.map((tag, index) => (
                    <span key={tag}>{index === 0 ? tag : `• ${tag}`}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.05] backdrop-blur-md">
            <div className="grid gap-8 p-8 md:p-12 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
              <div className="space-y-5">
                ...
                <h2 className="text-4xl font-semibold tracking-[-0.05em] text-white md:text-5xl">{siteConfig.home.contactTitle}</h2>
                <p className="max-w-2xl text-base leading-relaxed text-white/68 md:text-lg">
                  {siteConfig.home.contactText}
                </p>
              </div>
              <div className="space-y-4">
                <a href={`mailto:${siteConfig.contact.email}`} className="flex items-center justify-between rounded-[1.25rem] border border-white/10 bg-black/30 px-5 py-4 text-white/86 transition hover:bg-black/42">
                  <span className="flex items-center gap-3"><Mail className="h-4 w-4" /> {siteConfig.contact.email}</span>
                  <ArrowRight className="h-4 w-4" />
                </a>
                <a href={siteConfig.contact.instagramUrl} className="flex items-center justify-between rounded-[1.25rem] border border-white/10 bg-black/30 px-5 py-4 text-white/86 transition hover:bg-black/42">
                  <span className="flex items-center gap-3"><Instagram className="h-4 w-4" /> {siteConfig.contact.instagramLabel}</span>
                  <ArrowRight className="h-4 w-4" />
                </a>
              </div>
            </div>
